...
kb1 = client.get_or_create_collection(
    name="kb1"
...
def assemble_prompt(
    prompt_text,
    input_param_1_value=None,
    input_param_2_value=None,
    input_param_3_value=None,
    input_param_4_value=None,
    input_param_5_value=None,
    kb1_db=None,
    kb2_db=None,
    num_results=5,
):
...
selected_question = st.sidebar.selectbox(
    "Questions", questions, index=st.session_state.current_question_index
)
...
if uploaded_pdf and (
    st.session_state.knowledge_base_1 is None
    or uploaded_pdf.getvalue() != st.session_state.knowledge_base_1.getvalue()
):
...
matching_prompt_name_text = df[df["Prompt_ID"] == selected_question]["Prompt_ID"].iloc[
    0
]
...
st.markdown(
    f'<div style="text-align: right; font-size:28px; line-height:32px; margin-top:12px; margin-bottom:36px;">{matching_question_text}</div>',
...
model = st.selectbox("Model", ["gpt-3.5-turbo", "gpt-4", "gpt-4-1106-preview"], index=2)
temperature = st.slider(
    "Temperature", min_value=0.0, max_value=2.0, step=0.01, value=0.5
)
...
if st.button("Run Prompt"):
    formatted_prompt = assemble_prompt(
        prompt_text,
        optional_input_1,
        optional_input_2,
        optional_input_3,
